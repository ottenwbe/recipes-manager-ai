...
def calc_simple_similarity(recipe_id, num):
    ...
    recipe_similarities = _calc_jaccard_similarities(component_per_recipe[recipe_id], component_per_recipe)
    recipe_similarities = _post_process(recipe_similarities, num)
    similarity_per_recipe = {'id': recipe_id, 'similarities': recipe_similarities}

    return similarity_per_recipe
...
def _calc_jaccard_similarities(reference_recipe, component_per_recipe):
    ...
    for val in component_per_recipe.values():
        recipe_similarities = _append_recipe_similarity(reference_recipe, recipe_similarities, val)
...
def _append_recipe_similarity(reference_recipe, recipe_similarities, val):
    if reference_recipe['id'] != val['id']:
        score = _jaccard_similarity_set(reference_recipe['components'], val['components'])
...
def _get_recipe_components():
    ...
    for idx, recipe_id in enumerate(r.json()):
        r = requests.get(RECIPE_CONFIG.RECIPE_SERVER + '/api/v1/recipes/r/' + recipe_id)
        components = set()
        try:
        ...
        component_per_recipe[recipe_id] = {'id': recipe_id, 'components': components}
